...
if ($Register) {
  # An Interactive scheduled task that launches powershell.exe directly FLASHES a
  # black console window on every single run. Windows creates the console before
  # the shell can act on -WindowStyle Hidden, so "Hidden" never suppresses that
  # first flash -- it only hides the window that already appeared. At a 1-minute
  # interval that flash is very visible on the kiosk / desktop (owner 2026-07-18:
  # "a popup flashes every few minutes, disappears fast, ruins the experience";
  # it got worse the moment the interval went 5min -> 1min).
  # Fix: launch through a wscript.exe stub. wscript is a GUI-subsystem host with
  # no console of its own, and .Run(cmd, 0, False) passes SW_HIDE so the child
  # powershell is hidden FROM CREATION -- nothing ever flashes. Everything else
  # stays: still Interactive + elevated + every 1 min (tscon in step 0 still needs
  # the console session and admin rights); only the launch path changes.
  $vbsPath = Join-Path $PSScriptRoot 'run-hidden.vbs'
  $vbsBody = @'
' Hidden PowerShell launcher -- auto-generated by player-watchdog.ps1 -Register.
' Runs the .ps1 given as argument 0 with no visible window. Exists to avoid the
' once-a-minute black console flash an Interactive scheduled task produces when it
' launches powershell.exe directly; see the -Register block in player-watchdog.ps1.
...
'@
  ...
  $action = New-ScheduledTaskAction -Execute "$env:SystemRoot\System32\wscript.exe" `
    -Argument ('"' + $vbsPath + '" "' + $PSCommandPath + '"')
  ...
  Log 'Registered scheduled task SmartControl-PlayerWatchdog (every 1 min, interactive, elevated, hidden via wscript launcher)'
  exit 0
}
